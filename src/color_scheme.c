...
/*   Updated: 2017/07/01 19:47:27 by crenfrow         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void change_scheme(t_view *view, int id)
{
	if (id >= 0 && id < view->scheme_ct)
		...
		view->scheme_id = 0;
	t_list *l = ft_lstget(&view->schemes, id);
	printf("Scheme switched to %s - %d/%d\n", ((t_cscheme *) l->content)->title, view->scheme_id + 1, view->scheme_ct);
}
...
t_rgb color_for_escape(t_view *view, int vel)
{
	...
	if (sch)
	{
		color = *(t_rgb *) ft_lstget(&sch->colors, (view->max_iter / vel) * sch->color_ct)
		             ->content;
	}
	return (color);
}
